  ...
  if ((_83 < 1e-07f) || _110) {
    ...
    if (_5022 > 0.0f) {
      ...
      if (_119 < 8.0f) {
        ...
        float _rndxSiNdotL = mad(_109, _5140, mad(_108, _5135, (_107 * _5129)));
        ...
        while(true) {
          ...
          int2 _rndxSiSampleTexel = int2(int(SHADOW_CONTACT_SAMPLE_X(_5556, _5361) * _bufferSizeAndInvSize.x), int(_5555 * _bufferSizeAndInvSize.y));
          // RenoDX: <<< [Patch: ContactSelfIntersectionGuard]
          _5571 = __3__36__0__0__g_depthStencil.Load(int3(_rndxSiSampleTexel, 0));
          // RenoDX: <<< [Patch: ShadowEdgeNoiseFix]
          ...
          if (!(_5637 == 0)) {
            ...
            if (_rndxSiPlaneValid && (_5577 == _84)) {
            ...
            if (!_rndxSiReject) {
              ...
              if ((_rndxSiReceiverVulnerable) && ((_5577 == _84) && ((!_rndxCpeListA) && (!_rndxCpeTerrain)))) {
                if (!_rndxCpeReady) {
                  _rndxCpeReady = true;
                  float2 _rndxCpeDirPx = float2((_5350) * _bufferSizeAndInvSize.x, (_5352) * _bufferSizeAndInvSize.y);
                ...
                if (_rndxCpeEst) {
                  float _rndxCpeCodeLin = ((_5583) * (_5583)) / (_nearFarProj.x * 16777216.0f);
                  float _rndxCpeSepCodes = abs(_5586) / max(_rndxCpeCodeLin, 9.999999960041972e-13f);
            ...
            if (!_rndxSiReject) {
              float _microFarAccum = (saturate(1.0f - ((_5666 * _5666) * _5661)) * (1.0f - _5551)) * saturate((-0.0f - _5586) / (_5557 * 0.004654859658330679f)) * _rndxCpeFactor;
            ...
            _5682 = select(_rndxSiReject, _5553, _5577);
            // RenoDX: <<< [Patch: ContactSelfIntersectionGuard]
          } else {
          ...
          if ((!_rndxSiReject) && ((_5577 != 0) && (_5681 > 0.95f))) {
          // RenoDX: <<< [Patch: ContactSelfIntersectionGuard]
            if (!_5578) {
