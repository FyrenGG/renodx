...
float4 main(
  ...
  if (_549) {
    ...
    float3 _rndx_tonemapped_color = TonemapReplacer(float3(_526, _529, _532));
    _797 = _rndx_tonemapped_color.x;
    _804 = _rndx_tonemapped_color.y;
    _811 = _rndx_tonemapped_color.z;
  ...
  if (_etcParams.y > 1.0f) {
    ...
    _915 = saturate(1.0f - (dot(float2(_907, _909), float2(_907, _909)) * _rndx_vignette_strength));  // [sem: expr_sat]
  ...
  if (CUSTOM_BASIC_POSTPROCESS_FINAL == 1.f && !(_etcParams.z > 0.0f)) {
    float3 _rndx_final_color = FinalizeSDR(float3(_967, _968, _969), _sunDirection.y, _moonDirection.y);
    _967 = _rndx_final_color.x;
    _968 = _rndx_final_color.y;
    _969 = _rndx_final_color.z;
  }
