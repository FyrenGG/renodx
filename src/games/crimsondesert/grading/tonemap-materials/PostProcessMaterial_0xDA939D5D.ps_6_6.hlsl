...
float4 main(
  ...
  if (_etcParams.y > 1.0f) {
    ...
    _943 = saturate(1.0f - (dot(float2(_938, _939), float2(_938, _939)) * _rndx_vignette_strength));  // [sem: expr_sat]
  ...
  if (CUSTOM_BASIC_POSTPROCESS_FINAL == 1.f && !(_etcParams.z > 0.0f)) {
    float3 _rndx_final_color = FinalizeSDR(float3(_996, _997, _998), _sunDirection.y, _moonDirection.y);
    _996 = _rndx_final_color.x;
    _997 = _rndx_final_color.y;
    _998 = _rndx_final_color.z;
  }
