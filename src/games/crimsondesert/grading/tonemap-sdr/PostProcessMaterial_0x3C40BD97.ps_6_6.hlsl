...
float4 main(
  ...
  // [sem: _3__36__0__0__g_specularAO_sampleLod]
        ...
        _444 = (((uint2)(__3__36__0__0__g_stencil.Load(int3((int)(uint(_bufferSizeAndInvSize.x * TEXCOORD.x)), (int)(uint(_bufferSizeAndInvSize.y * TEXCOORD.y)), 0)))).x) & 127;
        _449 = (float)((bool)((uint)((_444 != 57) && ((uint)(_444 + -53) > (uint)14))));
        _462 = _449 * _413;
  ...
  if (_595) {
    ...
    // Description: HDR/PQ-output compile permutation of the PostProcessKnowledgeGain composite (padxil pair of 0x1E5F79F5, which received the same treatment for the SDR lane). It can own the scene during the location-discovery effect, so its vanilla tonemap branch would render the whole screen with the vanilla curve while every neighboring pass uses the RenoDX tonemap - visible as a flat grey/washed image for the duration of the effect. Replace the vanilla slope/offset/power grade, log-space tone curve, night tone adjustment, and output matrix with the shared TonemapReplacer, matching the treatment of the other mapped HDR-permutation PostProcessMaterial siblings. The vanilla _etcParams.z == 0 final-output suite below is intentionally preserved.
