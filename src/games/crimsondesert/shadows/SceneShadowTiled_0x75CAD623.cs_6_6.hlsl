  ...
  if ((_80 < 1e-07f) || _108) {
    ...
    if (_1604 > 0.0f) {
      ...
      if (CONTACT_SHADOW_DETAIL_PATH == 1.f) {
        _1730 = (int)(lerp(float(_1730), 16.0f, CONTACT_SHADOW_MARCH_BLEND) + 0.5f);
      }
      // RenoDX: <<< [Patch: ContactMicroShadowsFamily]
      if (!_171) {
      ...
      _1742 = ((abs(_1635) * (select(_1617, 12.0f, 2.0f) - _1618)) + _1618) * select(_171, lerp(0.01f, 0.05f, CONTACT_SHADOW_REACH_BLEND), 0.1f);
      // RenoDX: <<< [Patch: ContactMicroShadowsFamily]
      if (!_171) {
      ...
      if ((_81 == 15) && (!(_1616 || ((_81 != 15) && ((uint)((int)(_81) + (int)(-12)) < (uint)7))))) {
        ...
        //              exactly.
        _1940 = ((lerp((10.0f - (saturate(_116 * 0.001f) * 9.0f)), 2.0f, CONTACT_SHADOW_START_BLEND)) * _1928);
        // RenoDX: <<< [Patch: ContactMicroShadowsFamily]
      } else {
        _1940 = _1928;
      }
      ...
      _rndxMicroDirX = _1634;
      _rndxMicroDirY = _1635;
      _rndxMicroDirZ = _1636;
      _rndxMicroWorldPosX = _1952;
      _rndxMicroWorldPosY = _1953;
      _rndxMicroWorldPosZ = _1954;
      // RenoDX: <<< [Patch: ContactMicroShadowsFamily]
      ...
      while(true) {
        ...
        if (!(_2179 == 0)) {
          ...
          if (MICRO_SHADOW_FLICKER_FIX != 0.f && !_2123) {
            float _rndxJitterPx = (length(_temporalAAJitter.xy - _temporalAAJitter.zw) > 0.0f) ? 1.0f : 0.0f;
            float _rndxStepPx = max(length(float2((_2103 * _2080) * _bufferSizeAndInvSize.x, (_2103 * _2082) * _bufferSizeAndInvSize.y)), 1.0f);
            float _rndxGradPerPx = abs(_2121 - _2106) / _rndxStepPx;
            float _rndxBand = _rndxGradPerPx * _rndxJitterPx;
            float _rndxMargin = _2077 - abs((_2101 + _2077) - _2121);
            _rndxEvidenceW = (_rndxBand > 0.0f) ? saturate(_rndxMargin / _rndxBand) : 1.0f;
          }
          float _rndxMicroFarAccum = (saturate(1.0f - ((_2207 * _2207) * _2202)) * (1.0f - _2107)) * saturate((-0.0f - _2131) / (_2102 * 0.0046548597f)) * _rndxEvidenceW;
          _2223 = saturate((_rndxMicroFarAccum * lerp(1.0f, 0.7f, CONTACT_SHADOW_BASE_TUNING)) + _2107);
          // RenoDX: <<< [Patch: ContactMicroShadowsFamily]
        } else {
          _2222 = _2105;
    ...
    //              Contact Micro Shadows is Off, so the vanilla shadow value is preserved.
    //              The stencil argument is the masked material id (raw depth byte & 127), matching the
    //              other variants and the mask the helper applies to the ids it samples. The raw byte
    //              carries flag bits above the id, so passing it unmasked made the helper's
    //              excluded-material list silently miss any pixel with a flag bit set. The whole
    ...
    if (_1604 > 0.0f) {
      _2456 = ApplyContactMicroDetailShadow(
          _2456,
          float2(_61, _62),
          _116,
          _81,
      ...
      if (CONTACT_SHADOW_DETAIL_PATH == 1.f && _2456 < 1.0f) {
        ...
        _2456 = lerp(lerp(1.0f, _2456, 0.5f), _2456, _rndxMicroEdgeFade);
      }
    }
    // RenoDX: <<< [Patch: ContactMicroShadowsFamily]
