...
float4 main(
  ...
  // [sem: _3__36__0__0__g_specularAO_sampleLod]
        ...
        _408 = (((uint2)(__3__36__0__0__g_stencil.Load(int3((int)(uint(_bufferSizeAndInvSize.x * TEXCOORD.x)), (int)(uint(_bufferSizeAndInvSize.y * TEXCOORD.y)), 0)))).x) & 127;
        _413 = (float)((bool)((uint)((_408 != 57) && ((uint)(_408 + -53) > (uint)14))));
        _426 = _413 * _377;
  ...
  if (_523) {
    // RenoDX: >>> [Patch: KnowledgeGainTonemapReplace] [Version: 1.12.02]
    // Description: This PostProcessKnowledgeGain composite variant takes over the scene between the main composite material and the standalone final pass while the location-discovery (knowledge gain) screen effect plays, so its vanilla tonemap branch rendered the whole screen with the vanilla curve while every neighboring pass used the RenoDX tonemap. The image went flat and grey for the duration of the effect and snapped back to the RenoDX look when the pass stopped drawing. Replace the vanilla slope/offset/power grade, log-space tone curve, night tone adjustment, and output matrix with the shared TonemapReplacer used by the sibling PostProcessMaterial shaders (0x21212A93, 0x21A05DE2). The vanilla final-output suite in the _etcParams.z == 0 branch below (screen fade, wash, user image adjust, color-blind matrix) is intentionally preserved.
    ...
    // RenoDX: <<< [Patch: KnowledgeGainTonemapReplace]
  ...
  if (_etcParams.y > 1.0f) {
    ...
    // RenoDX: >>> [Patch: KnowledgeGainVignette] [Version: 1.12.02]
    // Description: When this KnowledgeGain composite variant is the visible final on the SDR DLAA lane, scale its native vignette by the RenoDX Vignette setting so the vignette state matches the main composite's behavior during the location-discovery effect instead of snapping to full native strength. Mirrors BasicPostProcessVignette in PostProcessMaterial_0x21212A93.
    float _rndx_vignette_strength = saturate(_etcParams.y + -1.0f);
    if (CUSTOM_BASIC_POSTPROCESS_FINAL == 1.f && !(_etcParams.z > 0.0f)) {
      _rndx_vignette_strength *= CUSTOM_VIGNETTE;
    }
    _892 = saturate(1.0f - (dot(float2(_887, _888), float2(_887, _888)) * _rndx_vignette_strength));
    // RenoDX: <<< [Patch: KnowledgeGainVignette]
    _897 = (_892 * _876);
    _898 = (_892 * _877);
    _899 = (_892 * _878);
  } else {
  ...
  // RenoDX: >>> [Patch: KnowledgeGainFinalizeSDR] [Version: 1.12.02]
  // Description: On the SDR DLAA lane this KnowledgeGain composite variant can be the visible final output during the location-discovery effect (no standalone SDR final draws and this shader writes the display target with _etcParams.z == 0, confirmed by live snapshot: chain 0x21212A93 -> 0x21A05DE2 -> 0x1E5F79F5 with zero standalone-final draws). Without this block the RenoDX SDR finalization (white point/color temperature, Purkinje gating, and the SDR Gamma setting) dropped out for the duration of the effect and snapped back afterward. Mirrors the BasicPostProcessFinalizeSDR patch in PostProcessMaterial_0x21212A93.
  if (CUSTOM_BASIC_POSTPROCESS_FINAL == 1.f && !(_etcParams.z > 0.0f)) {
  ...
  // RenoDX: <<< [Patch: KnowledgeGainFinalizeSDR]
  SV_Target.x = _945;
  SV_Target.y = _946;
  SV_Target.z = _947;
  SV_Target.w = _520;
