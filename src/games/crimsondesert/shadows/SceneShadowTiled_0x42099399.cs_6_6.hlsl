  ...
  if ((_84 < 1e-07f) || _111) {
    ...
    if (_1593 > 0.0f) {
      ...
      if (CONTACT_SHADOW_DETAIL_PATH == 1.f) {
        _1714 = (int)(lerp(float(_1714), 16.0f, CONTACT_SHADOW_MARCH_BLEND) + 0.5f);
      }
      // RenoDX: <<< [Patch: ContactMicroShadowsFamily]
      if (!_169) {
      ...
      _1727 = ((abs(_1618) * (select(_1599, 12.0f, 2.0f) - _1600)) + _1600) * select(_169, lerp(0.009999999776482582f, 0.05000000074505806f, CONTACT_SHADOW_REACH_BLEND), 0.10000000149011612f);
      // RenoDX: <<< [Patch: ContactMicroShadowsFamily]
      if (!_169) {
      ...
      if (_85 == 15) {
        ...
        //              exactly.
        _1912 = ((lerp((10.0f - (saturate(_120 * 0.0010000000474974513f) * 9.0f)), 2.0f, CONTACT_SHADOW_START_BLEND)) * _1903);
        // RenoDX: <<< [Patch: ContactMicroShadowsFamily]
      } else {
        _1912 = _1903;
      }
      ...
      _rndxMicroDirX = _1619;
      _rndxMicroDirY = _1618;
      _rndxMicroDirZ = _1617;
      _rndxMicroWorldPosX = _1924;
      _rndxMicroWorldPosY = _1925;
      _rndxMicroWorldPosZ = _1926;
      // RenoDX: <<< [Patch: ContactMicroShadowsFamily]
      ...
      while(true) {
        ...
        if (!(_2173 == 0)) {
          ...
          //              for bit.
          float _rndxMicroFarAccum = (saturate(1.0f - ((_2202 * _2202) * _2197)) * (1.0f - _2087)) * saturate((-0.0f - _2122) / (_2092 * 0.004654859658330679f));
          _2217 = saturate((_rndxMicroFarAccum * lerp(1.0f, 0.7f, CONTACT_SHADOW_BASE_TUNING)) + _2087);
          // RenoDX: <<< [Patch: ContactMicroShadowsFamily]
          _2218 = _2113;
        } else {
    ...
    if (_1593 > 0.0f) {
      _2619 = ApplyContactMicroDetailShadow(
          _2619,
          float2(_65, _66),
          _120,
          _85,
      ...
      if (CONTACT_SHADOW_DETAIL_PATH == 1.f && _2619 < 1.0f) {
        ...
        _2619 = lerp(lerp(1.0f, _2619, 0.5f), _2619, _rndxMicroEdgeFade);
      }
    }
    // RenoDX: <<< [Patch: ContactMicroShadowsFamily]
