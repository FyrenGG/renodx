...
float4 main(
  ...
  if (_1833) {
    ...
    // Description: This is the SDR permutation of screen-effect composite variant 0xE94A87D2 (its bindless parameter struct is PostProcessLensFlareForGimmick_CD, a quest-gimmick lens/sun-flare overlay pass); like every member of the PostProcessMaterial family it statically inlines the full vanilla tonemap pipeline and can own the visible final output while its screen effect plays, so an unreplaced curve renders the whole screen with vanilla colors (flat/grey versus the RenoDX look) for the duration of the effect. This block replaces the vanilla slope/offset/power grade, log-space tone curve, night tone adjustment, and output matrix with the shared TonemapReplacer while preserving the vanilla _etcParams.z == 0 final-output suite below (screen fade, wash, user image adjust, color-blind matrix).
  ...
  if (_etcParams.y > 1.0f) {
    ...
    // Description: When this SDR screen-effect composite variant (0xE94A87D2, gimmick lens-flare overlay) is the visible final output while its effect plays, scale its native vignette strength by the RenoDX Vignette setting so vignette intensity stays consistent with the main composite instead of snapping to full native strength for the effect's duration. Only applies when the vanilla sRGB-encode path is inactive (_etcParams.z == 0), i.e. when this shader owns the display output. Mirrors the BasicPostProcessVignette patch in PostProcessMaterial_0x21212A93.
    float _rndx_vignette_strength = saturate(_etcParams.y + -1.0f);
    if (CUSTOM_BASIC_POSTPROCESS_FINAL == 1.f && !(_etcParams.z > 0.0f)) {
      _rndx_vignette_strength *= CUSTOM_VIGNETTE;
    }
    _2202 = saturate(1.0f - (dot(float2(_2197, _2198), float2(_2197, _2198)) * _rndx_vignette_strength));
  ...
  // Description: On the SDR lane this screen-effect composite variant (0xE94A87D2, gimmick lens-flare overlay) can be the visible final output while its effect plays: it writes the display target directly when _etcParams.z == 0 and no standalone SDR final pass draws after it. Without this block the RenoDX white point/color-temperature adjustment, Purkinje night handling, and the SDR Gamma finalization drop out for the duration of the effect and snap back when it stops drawing. Applies FinalizeSDR to the post-letterbox color exactly as the BasicPostProcessFinalizeSDR patch does in PostProcessMaterial_0x21212A93.
  if (CUSTOM_BASIC_POSTPROCESS_FINAL == 1.f && !(_etcParams.z > 0.0f)) {
    float3 _rndx_final_color = FinalizeSDR(float3(_2255, _2256, _2257), _sunDirection.y, _moonDirection.y);
    _2255 = _rndx_final_color.x;
    _2256 = _rndx_final_color.y;
    _2257 = _rndx_final_color.z;
  }
