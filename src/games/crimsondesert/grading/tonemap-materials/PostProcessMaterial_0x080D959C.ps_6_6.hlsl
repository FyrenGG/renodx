...
float4 main(
  ...
  if (_178) {
    ...
    float3 _rndx_tonemapped_color = TonemapReplacer(float3(_158, _159, _160));
    _426 = _rndx_tonemapped_color.x;
    _433 = _rndx_tonemapped_color.y;
    _440 = _rndx_tonemapped_color.z;
  ...
  if (_etcParams.y > 1.0f) {
    ...
    _544 = saturate(1.0f - (dot(float2(_536, _538), float2(_536, _538)) * _rndx_vignette_strength));  // [sem: expr_sat]
  ...
  if (CUSTOM_BASIC_POSTPROCESS_FINAL == 1.f && !(_etcParams.z > 0.0f)) {
    float3 _rndx_final_color = FinalizeSDR(float3(_597, _598, _599), _sunDirection.y, _moonDirection.y);
    _597 = _rndx_final_color.x;
    _598 = _rndx_final_color.y;
    _599 = _rndx_final_color.z;
  }
