...
float4 main(
  ...
  // [sem: _3__36__0__0__g_CustomRenderPassValue_gather]
    ...
    _1794 = 1.0f - (saturate(_354 * 7.0f) * 0.9950000047683716f);
    _1795 = _1794 * _1787.x;
    _1796 = _1794 * _1787.y;
    _1797 = _1794 * _1787.z;
    if (!_1785) {
    ...
    _1808 = (_1801 * (max(0.0f, (_1734 * (((_1222 * saturate(_1208)) + ((_322 * 0.5f) * _333)) + (_1724 * exp2(log2(((((_1688 + (_1426 * 0.033104754984378815f)) + (((_1622 * 0.16689524054527283f) + 0.033104754984378815f) * _1622)) + _1676) + (_1686 * 0.07323896884918213f)) * 2.0f)))))) - _1795)) + _1795;
    _1809 = (_1801 * (max(0.0f, (_1734 * (((_1222 * saturate(_1209)) + ((_323 * 0.699999988079071f) * _333)) + (_1724 * exp2(log2(((((_1688 + (_1694 * 0.033104754984378815f)) + ((0.07323896884918213f - (_1622 * 0.02323896810412407f)) * _1622)) + _1676) + (_1686 * 0.21404112875461578f)) * 2.0f)))))) - _1796)) + _1796;
    _1810 = (_1801 * (max(0.0f, (_1734 * (((_1222 * saturate(_1210)) + ((_324 * 0.8999999761581421f) * _333)) + (_1724 * exp2(log2((((((((_1694 * 0.07323896884918213f) + (_1390 * 0.033104754984378815f)) + ((0.13286834955215454f - (_1622 * 0.08286835253238678f)) * _1622)) + (_1665 * 0.06370936334133148f)) + (_1662 * 0.1098584532737732f)) + (_1671 * 0.6038274168968201f)) + _1676) * 2.0f)))))) - _1797)) + _1797;
    if (!_1785) {
    ...
    if (_1831) {
      ...
      // Description: This shader (hash 0xBF1DCB47, SDR permutation) is a screen-effect composite variant of the game's post-process material family (its bindless parameter struct is PostProcessAbyssEnterGlid_CD, an abyss-entry glide transition overlay). Every member of this family statically inlines the full vanilla tonemap-and-final pipeline and can own the visible final output while its screen effect plays, so an unreplaced vanilla curve renders the whole screen with the vanilla tonemap (flat/grey compared to the RenoDX look) for the effect's duration. This block replaces the vanilla slope/offset/power grade, log-space tone curve, night tone adjustment, and output matrix with the shared TonemapReplacer while preserving the vanilla _etcParams.z == 0 final-output suite (screen fade, wash, user image adjust, color-blind matrix) below.
      float3 _rndx_tonemapped_color = TonemapReplacer(float3(_1808, _1809, _1810));
      _2089 = _rndx_tonemapped_color.x;
      _2090 = _rndx_tonemapped_color.y;
      _2091 = _rndx_tonemapped_color.z;
      ...
      if (_etcParams.z == 0.0f) {
        ...
        _2171 = 2.200000047683716f / ((min(max(_userImageAdjust.w, -1.0f), 1.0f) * 0.800000011920929f) + 2.200000047683716f);
    ...
    if (_etcParams.y > 1.0f) {
      ...
      // Description: When this screen-effect composite variant (0xBF1DCB47, SDR permutation) is the visible final output on the SDR lane (_etcParams.z == 0) while its effect plays, scale the shader's native vignette strength by the RenoDX Vignette setting so vignette behavior matches the main composite material instead of snapping to full native strength for the effect's duration. Mirrors the BasicPostProcessVignette patch in PostProcessMaterial_0x21212A93.
      float _rndx_vignette_strength = saturate(_etcParams.y + -1.0f);
      if (CUSTOM_BASIC_POSTPROCESS_FINAL == 1.f && !(_etcParams.z > 0.0f)) {
        _rndx_vignette_strength *= CUSTOM_VIGNETTE;
      }
      _2196 = saturate(1.0f - (dot(float2(_2191, _2192), float2(_2191, _2192)) * _rndx_vignette_strength));  // [sem: expr_sat]
    ...
    if (_1831 && (_etcParams.z > 0.0f)) {
      _2233 = select((_2201 <= 0.0031308000907301903f), (_2201 * 12.920000076293945f), (((pow(_2201, 0.4166666567325592f)) * 1.0549999475479126f) + -0.054999999701976776f));
      _2234 = select((_2202 <= 0.0031308000907301903f), (_2202 * 12.920000076293945f), (((pow(_2202, 0.4166666567325592f)) * 1.0549999475479126f) + -0.054999999701976776f));
      _2235 = select((_2203 <= 0.0031308000907301903f), (_2203 * 12.920000076293945f), (((pow(_2203, 0.4166666567325592f)) * 1.0549999475479126f) + -0.054999999701976776f));
    } else {
      _2233 = _2201;
      _2234 = _2202;
      _2235 = _2203;
    ...
    // Description: On the SDR lane this screen-effect composite variant (0xBF1DCB47) can be the visible final output while its effect plays: it writes the display target with _etcParams.z == 0, so no later pass reapplies the RenoDX finalization. Without this block the RenoDX white point/color-temperature handling, Purkinje night adjustment, and the SDR Gamma setting would drop out for the duration of the effect and snap back afterward. Applies FinalizeSDR to the post-letterbox color, gated to the case where this shader is the final SDR output. Mirrors the BasicPostProcessFinalizeSDR patch in PostProcessMaterial_0x21212A93.
    if (CUSTOM_BASIC_POSTPROCESS_FINAL == 1.f && !(_etcParams.z > 0.0f)) {
      float3 _rndx_final_color = FinalizeSDR(float3(_2249, _2250, _2251), _sunDirection.y, _moonDirection.y);
      _2249 = _rndx_final_color.x;
      _2250 = _rndx_final_color.y;
      _2251 = _rndx_final_color.z;
    }
