...
float4 main(
  ...
  if (_578) {
    ...
    float3 _rndx_tonemapped_color = TonemapReplacer(float3(_561, _560, _559));
  ...
  if (_etcParams.y > 1.0f) {
    ...
    _947 = saturate(1.0f - (dot(float2(_942, _943), float2(_942, _943)) * _rndx_vignette_strength));  // [sem: expr_sat]
  ...
  if (CUSTOM_BASIC_POSTPROCESS_FINAL == 1.f && !(_etcParams.z > 0.0f)) {
    float3 _rndx_final_color = FinalizeSDR(float3(_1000, _1001, _1002), _sunDirection.y, _moonDirection.y);
    _1000 = _rndx_final_color.x;
    _1001 = _rndx_final_color.y;
    _1002 = _rndx_final_color.z;
  }
