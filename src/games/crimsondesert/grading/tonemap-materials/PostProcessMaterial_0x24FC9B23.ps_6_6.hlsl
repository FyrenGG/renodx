...
float4 main(
  ...
  while(true) {
    ...
    if (_405) {
      ...
      float3 _rndx_tonemapped_color = TonemapReplacer(float3(_382, _385, _388));
      _653 = _rndx_tonemapped_color.x;
      _660 = _rndx_tonemapped_color.y;
      _667 = _rndx_tonemapped_color.z;
    ...
    if (_etcParams.y > 1.0f) {
      ...
      _771 = saturate(1.0f - (dot(float2(_763, _765), float2(_763, _765)) * _rndx_vignette_strength));  // [sem: expr_sat]
    ...
    if (CUSTOM_BASIC_POSTPROCESS_FINAL == 1.f && !(_etcParams.z > 0.0f)) {
      float3 _rndx_final_color = FinalizeSDR(float3(_827, _828, _829), _sunDirection.y, _moonDirection.y);
      _827 = _rndx_final_color.x;
      _828 = _rndx_final_color.y;
      _829 = _rndx_final_color.z;
    }
