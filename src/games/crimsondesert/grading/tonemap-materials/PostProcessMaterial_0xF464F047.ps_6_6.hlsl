...
float4 main(
  ...
  if (_etcParams.y > 1.0f) {
    ...
    _605 = saturate(1.0f - (dot(float2(_596, _599), float2(_596, _599)) * _rndx_vignette_strength));  // [sem: expr_sat]
  ...
  if (CUSTOM_BASIC_POSTPROCESS_FINAL == 1.f && !(_etcParams.z > 0.0f)) {
    float3 _rndx_final_color = FinalizeSDR(float3(_658, _659, _660), _sunDirection.y, _moonDirection.y);
    _658 = _rndx_final_color.x;
    _659 = _rndx_final_color.y;
    _660 = _rndx_final_color.z;
  }
