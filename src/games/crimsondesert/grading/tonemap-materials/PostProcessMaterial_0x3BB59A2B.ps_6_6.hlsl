...
float4 main(
  ...
  if (_97) {
    ...
    float3 _rndx_tonemapped_color = TonemapReplacer(float3(_73, _76, _79));
    _345 = _rndx_tonemapped_color.x;
    _352 = _rndx_tonemapped_color.y;
    _359 = _rndx_tonemapped_color.z;
  ...
  if (_etcParams.y > 1.0f) {
    ...
    _466 = saturate(1.0f - (dot(float2(_457, _460), float2(_457, _460)) * _rndx_vignette_strength));  // [sem: expr_sat]
  ...
  if (CUSTOM_BASIC_POSTPROCESS_FINAL == 1.f && !(_etcParams.z > 0.0f)) {
    float3 _rndx_final_color = FinalizeSDR(float3(_519, _520, _521), _sunDirection.y, _moonDirection.y);
    _519 = _rndx_final_color.x;
    _520 = _rndx_final_color.y;
    _521 = _rndx_final_color.z;
  }
