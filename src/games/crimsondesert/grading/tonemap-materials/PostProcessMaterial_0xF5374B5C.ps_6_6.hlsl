...
float4 main(
  ...
  while(true) {
    ...
    if (_2401) {
      ...
      float3 _rndx_tonemapped_color = TonemapReplacer(float3(_2378, _2381, _2384));
      _2649 = _rndx_tonemapped_color.x;
      _2656 = _rndx_tonemapped_color.y;
      _2663 = _rndx_tonemapped_color.z;
    ...
    if (_etcParams.y > 1.0f) {
      ...
      _2767 = saturate(1.0f - (dot(float2(_2759, _2761), float2(_2759, _2761)) * _rndx_vignette_strength));  // [sem: expr_sat]
    ...
    if (CUSTOM_BASIC_POSTPROCESS_FINAL == 1.f && !(_etcParams.z > 0.0f)) {
      float3 _rndx_final_color = FinalizeSDR(float3(_2820, _2821, _2822), _sunDirection.y, _moonDirection.y);
      _2820 = _rndx_final_color.x;
      _2821 = _rndx_final_color.y;
      _2822 = _rndx_final_color.z;
    }
