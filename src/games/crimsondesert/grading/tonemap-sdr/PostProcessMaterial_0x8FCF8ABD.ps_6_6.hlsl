...
float4 main(
  ...
  if (_1833) {
    ...
    // Description: HDR/PQ-output compile permutation of the PostProcessLensFlareForGimmick screen-effect composite (hash 0x8FCF8ABD; its bindless parameter block drives a gimmick/sun lens-flare overlay drawn as a full-screen postprocess material pass). Members of this postprocess material family statically inline the vanilla tonemap chain and can own the visible final image while their effect plays, so an unreplaced curve renders the whole screen with vanilla colors (flat/grey versus the RenoDX look) for the duration of the effect. This block replaces the vanilla slope/offset/power grade, log-space tone curve, night tone adjustment, and output matrix with the shared TonemapReplacer while preserving the vanilla _etcParams.z == 0 final-output suite below.
