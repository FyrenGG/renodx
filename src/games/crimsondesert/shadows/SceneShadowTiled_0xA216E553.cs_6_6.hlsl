  ...
  if ((_81 < 1e-07f) || _108) {
    ...
    if (_1274 > 0.0f) {
      ...
      if (CONTACT_SHADOW_DETAIL_PATH == 1.f) {
        _1393 = (int)(lerp(float(_1393), 16.0f, CONTACT_SHADOW_MARCH_BLEND) + 0.5f);
      }
      // RenoDX: <<< [Patch: ContactMicroShadowsFamily]
      if (!_166) {
      ...
      _1406 = ((abs(_1299) * (select(_1280, 12.0f, 2.0f) - _1281)) + _1281) * select(_166, lerp(0.009999999776482582f, 0.05000000074505806f, CONTACT_SHADOW_REACH_BLEND), 0.10000000149011612f);
      // RenoDX: <<< [Patch: ContactMicroShadowsFamily]
      if (!_166) {
      ...
      if (_82 == 15) {
        ...
        //              exactly.
        _1456 = ((lerp((10.0f - (saturate(_117 * 0.0010000000474974513f) * 9.0f)), 2.0f, CONTACT_SHADOW_START_BLEND)) * _1447);
        // RenoDX: <<< [Patch: ContactMicroShadowsFamily]
      } else {
        _1456 = _1447;
      }
      ...
      _rndxMicroDirX = _1300;
      _rndxMicroDirY = _1299;
      _rndxMicroDirZ = _1298;
      _rndxMicroWorldPosX = _1468;
      _rndxMicroWorldPosY = _1469;
      _rndxMicroWorldPosZ = _1470;
      // RenoDX: <<< [Patch: ContactMicroShadowsFamily]
      ...
      while(true) {
        ...
        if (!(_1713 == 0)) {
          ...
          float _rndxMicroFarAccum = (saturate(1.0f - ((_1742 * _1742) * _1737)) * (1.0f - _1627)) * saturate((-0.0f - _1662) / (_1632 * 0.004654859658330679f));
          _1757 = saturate((_rndxMicroFarAccum * lerp(1.0f, 0.7f, CONTACT_SHADOW_BASE_TUNING)) + _1627);
          // RenoDX: <<< [Patch: ContactMicroShadowsFamily]
          _1758 = _1653;
        } else {
    ...
    if (_1274 > 0.0f) {
      _2119 = ApplyContactMicroDetailShadow(
          _2119,
          float2(_62, _63),
          _117,
          _82,
      ...
      if (CONTACT_SHADOW_DETAIL_PATH == 1.f && _2119 < 1.0f) {
        ...
        _2119 = lerp(lerp(1.0f, _2119, 0.5f), _2119, _rndxMicroEdgeFade);
      }
    }
    // RenoDX: <<< [Patch: ContactMicroShadowsFamily]
