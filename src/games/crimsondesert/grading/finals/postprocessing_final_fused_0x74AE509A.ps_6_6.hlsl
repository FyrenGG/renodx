Texture3D<float4> __3__36__0__0__g_displayRenderingTransformLUT : register(t146, space36);
...
float4 main(
  ...
  _664 = saturate(_etcParams.w);
  _665 = (_660 * saturate(_rndx_tonemapped_color.x)) + _664;
  _666 = (_660 * saturate(_rndx_tonemapped_color.y)) + _664;
  _667 = (_660 * saturate(_rndx_tonemapped_color.z)) + _664;
  ...
  _698 = (pow(_689, 0.012683313339948654f));
  _699 = (pow(_690, 0.012683313339948654f));
  _700 = (pow(_691, 0.012683313339948654f));
  _726 = (TEXCOORD.x * 2.0f) + -1.0f;
  _727 = TEXCOORD.y * 2.0f;
  _728 = 1.0f - _727;
  _756 = mad((_projToPrevProj[2].w), 1.0000000116860974e-07f, mad((_projToPrevProj[1].w), _728, ((_projToPrevProj[0].w) * _726))) + (_projToPrevProj[3].w);
  _759 = ((mad((_projToPrevProj[2].x), 1.0000000116860974e-07f, mad((_projToPrevProj[1].x), _728, ((_projToPrevProj[0].x) * _726))) + (_projToPrevProj[3].x)) / _756) - _726;
  _760 = ((mad((_projToPrevProj[2].y), 1.0000000116860974e-07f, mad((_projToPrevProj[1].y), _728, ((_projToPrevProj[0].y) * _726))) + (_projToPrevProj[3].y)) / _756) - _728;
  ...
  _777 = abs(_726);
  _778 = abs(_727 + -1.0f);
  _782 = saturate(1.0f - ((_774 * _postProcessParams.x * CUSTOM_VIGNETTE) * dot(float2(_777, _778), float2(_777, _778))));  // [sem: expr_sat]
  _792 = exp2(log2(_782 * exp2(log2(max(0.0f, (_698 + -0.8359375f)) / (18.8515625f - (_698 * 18.6875f))) * 6.277394771575928f)) * 0.1593017578125f);
  _793 = exp2(log2(_782 * exp2(log2(max(0.0f, (_699 + -0.8359375f)) / (18.8515625f - (_699 * 18.6875f))) * 6.277394771575928f)) * 0.1593017578125f);
  _794 = exp2(log2(_782 * exp2(log2(max(0.0f, (_700 + -0.8359375f)) / (18.8515625f - (_700 * 18.6875f))) * 6.277394771575928f)) * 0.1593017578125f);
  _827 = (!(SV_Position.y < _viewDir.w)) && (!(SV_Position.y >= (_screenSizeAndInvSize.y - _viewDir.w)));
  SV_Target.x = select(_827, exp2(log2((1.0f / ((_792 * 18.6875f) + 1.0f)) * ((_792 * 18.8515625f) + 0.8359375f)) * 78.84375f), 0.0f);
  SV_Target.y = select(_827, exp2(log2((1.0f / ((_793 * 18.6875f) + 1.0f)) * ((_793 * 18.8515625f) + 0.8359375f)) * 78.84375f), 0.0f);
  SV_Target.z = select(_827, exp2(log2((1.0f / ((_794 * 18.6875f) + 1.0f)) * ((_794 * 18.8515625f) + 0.8359375f)) * 78.84375f), 0.0f);
  SV_Target.w = _17.w;
