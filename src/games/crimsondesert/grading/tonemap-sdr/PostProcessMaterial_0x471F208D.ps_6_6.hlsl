        ...
        while(true) {
          ...
          while(true) {
            ...
            if (_227) {
              ...
              // Description: This is screen-effect composite variant 0x471F208D, an SDR permutation of the PostProcessTransition screen-effect material family; it runs a variance-minimizing painterly filter over the scene color and weighs the result through a sepia-style matrix before tonemapping. It statically inlines the vanilla tonemap and can own the visible final output while its screen effect plays, so an unreplaced curve renders the whole screen with vanilla colors (flat/grey versus the RenoDX look) for the effect's duration. This block replaces the vanilla slope/offset/power grade, log-space tone curve, night tone adjustment, and output matrix with the shared TonemapReplacer while preserving the vanilla _etcParams.z == 0 final-output suite below. Unlike the common family shape, this variant folds a 0.3 pre-scale into the tonemap input matrix constants (0.51151/0.34224/0.34589 instead of 1.70505/1.14080/1.15297) and its non-tonemap fallback branch also multiplies by 0.3, so the replacement applies the same 0.3 scale to the sepia-weighted trio before calling TonemapReplacer.
            ...
            if (_etcParams.y > 1.0f) {
              ...
              // Description: When this screen-effect composite variant is the visible final on the SDR lane, scale its native vignette by the RenoDX Vignette setting so the vignette strength matches the main composite's behavior while the effect plays instead of snapping to full native strength. Mirrors the BasicPostProcessVignette patch in PostProcessMaterial_0x21212A93.
              float _rndx_vignette_strength = saturate(_etcParams.y + -1.0f);
              if (CUSTOM_BASIC_POSTPROCESS_FINAL == 1.f && !(_etcParams.z > 0.0f)) {
                _rndx_vignette_strength *= CUSTOM_VIGNETTE;
              }
              _596 = saturate(1.0f - (dot(float2(_591, _592), float2(_591, _592)) * _rndx_vignette_strength));
            ...
            // Description: On the SDR lane this screen-effect composite variant can be the visible final output while its effect plays (it writes the display target with _etcParams.z == 0 and no standalone SDR final pass draws after it). Without this block the RenoDX SDR finalization (white point/color temperature, Purkinje gating, and the SDR Gamma setting) dropped out for the duration of the effect and snapped back afterward. Mirrors the BasicPostProcessFinalizeSDR patch in PostProcessMaterial_0x21212A93.
            if (CUSTOM_BASIC_POSTPROCESS_FINAL == 1.f && !(_etcParams.z > 0.0f)) {
              float3 _rndx_final_color = FinalizeSDR(float3(_649, _650, _651), _sunDirection.y, _moonDirection.y);
              _649 = _rndx_final_color.x;
              _650 = _rndx_final_color.y;
              _651 = _rndx_final_color.z;
            }
