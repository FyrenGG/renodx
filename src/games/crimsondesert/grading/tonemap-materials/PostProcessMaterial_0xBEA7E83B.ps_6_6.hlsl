...
float4 main(
  ...
  if (_2324) {
    ...
    float3 _rndx_tonemapped_color = TonemapReplacer(float3(_2301, _2304, _2307));
    _2572 = _rndx_tonemapped_color.x;
    _2579 = _rndx_tonemapped_color.y;
    _2586 = _rndx_tonemapped_color.z;
  ...
  if (_etcParams.y > 1.0f) {
    ...
    _2693 = saturate(1.0f - (dot(float2(_2684, _2687), float2(_2684, _2687)) * _rndx_vignette_strength));  // [sem: expr_sat]
  ...
  if (CUSTOM_BASIC_POSTPROCESS_FINAL == 1.f && !(_etcParams.z > 0.0f)) {
    float3 _rndx_final_color = FinalizeSDR(float3(_2746, _2747, _2748), _sunDirection.y, _moonDirection.y);
    _2746 = _rndx_final_color.x;
    _2747 = _rndx_final_color.y;
    _2748 = _rndx_final_color.z;
  }
