      ...
      if (_115 < 8.0f) {
        ...
        while(true) {
          ...
          if (!(_5849 == 0)) {
            ...
            if (MICRO_SHADOW_FLICKER_FIX != 0.f && !_5793) {
              float _rndxJitterPx = (length(_temporalAAJitter.xy - _temporalAAJitter.zw) > 0.0f) ? 1.0f : 0.0f;
              float _rndxStepPx = max(length(float2((_5774 * _5750) * _bufferSizeAndInvSize.x, (_5774 * _5752) * _bufferSizeAndInvSize.y)), 1.0f);
              float _rndxGradPerPx = abs(_5791 - _5776) / _rndxStepPx;
              float _rndxBand = _rndxGradPerPx * _rndxJitterPx;
              float _rndxMargin = _5747 - abs((_5772 + _5747) - _5791);
              _rndxEvidenceW = (_rndxBand > 0.0f) ? saturate(_rndxMargin / _rndxBand) : 1.0f;
            }
            float _microNearAccum = (saturate(1.0f - ((_5877 * _5877) * _5872)) * (1.0f - _5777)) * saturate((-0.0f - _5801) / (_5773 * 0.0046548597f)) * _rndxEvidenceW;
        ...
        float _rndxFarWindowScale = (SHADOW_DISTANCE_SEAM_FIX != 0.f)
                                        ...
                                        : 0.5f;
        _6035 = ((_5405 * _rndxFarWindowScale) * max(abs(_6016), (_5996 - ((mad((_proj[2].z), _115, 0.0f) + _5967) / (mad((_proj[3].z), _115, 0.0f) + _5971))))) * max(0.0625f, _6004);
        ...
        while(true) {
          ...
          if (!(_6137 == 0)) {
            ...
            if (!_rndxSiReject) {
              float _rndxEvidenceW = 1.0f;
              if (MICRO_SHADOW_FLICKER_FIX != 0.f && !_6081) {
                float _rndxJitterPx = (length(_temporalAAJitter.xy - _temporalAAJitter.zw) > 0.0f) ? 1.0f : 0.0f;
                float _rndxStepPx = max(length(float2((_6058 * _6038) * _bufferSizeAndInvSize.x, (_6058 * _6040) * _bufferSizeAndInvSize.y)), 1.0f);
                float _rndxGradPerPx = abs(_6079 - _6064) / _rndxStepPx;
                float _rndxBand = _rndxGradPerPx * _rndxJitterPx;
                float _rndxMargin = _6035 - abs((_6062 + _6035) - _6079);
                _rndxEvidenceW = (_rndxBand > 0.0f) ? saturate(_rndxMargin / _rndxBand) : 1.0f;
              }
              float _microFarAccum = (saturate(1.0f - ((_6165 * _6165) * _6160)) * (1.0f - _6065)) * saturate((-0.0f - _6089) / (_6059 * 0.0046548597f)) * _rndxCpeFactor * _rndxEvidenceW;
